	...
	<div style="display: flex; justify-content: center;">
		<ul class="pagination pagination-lg">
            ...
            @if($products->currentPage() > 1)
                <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() - 1 }}">Previous</a></li>
            @endif
                <li class="page-item active"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() }}">{{ $products->currentPage() }}</a></li>
            @if($products->currentPage() < $products->lastPage())
                <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() + 1 }}">{{ $products->currentPage() + 1 }}</a></li>
                @if($products->currentPage() < $products->lastPage() - 2)
                <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() + 2 }}">{{ $products->currentPage() + 2 }}</a></li>
                @endif
                @if($products->currentPage() < $products->lastPage() - 3)
                <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() + 3 }}">{{ $products->currentPage() + 3 }}</a></li>
                @endif
            @endif
            {{-- nếu page hiện tại < page cuối thì hiện nút next --}}
            @if($products->currentPage() < $products->lastPage())
                <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->currentPage() + 1 }}">Next</a>
            @endif
            <li class="page-item"><a class="page-link" href="khuyen-mai?page={{ $products->lastPage() }}">Last</a></li>
		</ul>
	</div>
</div>
@stop
